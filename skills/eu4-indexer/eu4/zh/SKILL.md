...
- **令牌的特殊效果在入站引用里**：理念、理念组、政改、建筑、伟大工程、政策等"身份
  令牌"的修正块只是明面效果——真正的特殊机制在**别处用 `has_idea` 等检查它**的决议/
  任务/事件里。这些入站引用已并入 `refs`，直接用 `explain_entity`/`what_triggers`/
  `find_by_condition`(传理念/政改等的 key) 即可查到，无需手写 `script_nodes` 查询。
  （例：MFA 的 `MFA_byzantine_claimants` 理念会显出它解锁的 `restore_byzantine_empire` 决议。）
